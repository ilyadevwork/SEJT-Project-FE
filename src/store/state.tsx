...
import {
  tableDataType, rootIdxType, branchIdxType, justObjects, Dictionary,
...
export const disabledDate: RangePickerProps['disabledDate'] = (current) => {
  ...
  if (snapshots.find((obj) => moment(obj.date).format('YYYY-MM-DD') === current.format('YYYY-MM-DD')) !== undefined) return false;
  else return true;
};

export function buildTechIndex(curSnap: any): Dictionary<number> {
  let index: Dictionary<number> = {};
  for (let i = 0; i < snapshots[curSnap].techBranch.length; i++) index[snapshots[curSnap].techBranch[i].name] = i;
