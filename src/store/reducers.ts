...
export const toggleActionReducer = (
  state: configState,
  action: toggleAction
) => {
  switch (action.type) {
    case toggleActionType.TOGGLE_SERIES: {
      return {
        ...
        routeID: routeCalculator(
          state.isAggregate,
          state.isAllTechChecked,
          action.payload
        ),
      };
    }
...
// Series 0 Aggregate 0 All Tech 0 -> 0
// We can expect updates from date picker, category picker, subcategory picker
// and technologies list picker.

// Series 0 Aggregate 0 All Tech 1 -> 1

// Series 0 Aggregate 1 All Tech 1 -> 2

// Series 1 Aggregate 0 All Tech 0 -> 3

export const dataUpdateReducer = (
  state: configState,
  routeID: number,
  action: dataAction
) => {
  ...
  switch (routeID) {
        ...
        case dataActionType.SET_SUBCATEGORIES: {
          ...
          if (state.selectedTechnologies.length === 0) {
            ...
            if (action.payload > state.selectedSubCategories) {
              let tempChartData: object[] = [...state.chartData.data];
              let tempTableData: tableDataType[] = [...state.tableData];
    ...
    case 1: {
      // Series False, Aggregate False, All Tech True
      // Same format as case 0, but instead of user selected technologies we load
      // all availible.
...
function routeCalculator(
  ...
  if ((isAgg && isAllTech === true) || (isAgg === true && isAllTech === false))
    return 2;
